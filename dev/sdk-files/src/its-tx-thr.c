...
static int DENMTx_SendDENMTrigger_Events(struct ITSCtrl *pITS);
static int DENMTx_SendDENMTerminate_Events(struct ITSCtrl *pITS);
...
// DENM information
tDENMActionID DENMActionId1;
tDENMActionID DENMActionId2;
tDENMActionID DENMActionId3;
tDENMActionID DENMActionId4;
tDENMActionID DENMActionId5;
...
ITSDecentralizedEnvironmentalNotificationMessage *pDENMMsg1 = NULL;
ITSDecentralizedEnvironmentalNotificationMessage *pDENMMsg2 = NULL;
ITSDecentralizedEnvironmentalNotificationMessage *pDENMMsg3 = NULL;
ITSDecentralizedEnvironmentalNotificationMessage *pDENMMsg4 = NULL;
ITSDecentralizedEnvironmentalNotificationMessage *pDENMMsg5 = NULL;
...
{
    if ((pITS->Tx.DENM1.Enabled) || (pITS->Tx.IVI.Enabled))
    {
...
static void ITSTx_ThreadProc(void *pArg)
{
    ...
    if (pITS->Tx.DENM1.Enabled)
    {
        pDENMMsg1 = asn1_mallocz(sizeof(ITSDecentralizedEnvironmentalNotificationMessage));
    }

    if (pITS->Tx.DENM2.Enabled)
    {
        pDENMMsg2 = asn1_mallocz(sizeof(ITSDecentralizedEnvironmentalNotificationMessage));
    }

    if (pITS->Tx.DENM3.Enabled)
    {
        pDENMMsg3 = asn1_mallocz(sizeof(ITSDecentralizedEnvironmentalNotificationMessage));
    }

    if (pITS->Tx.DENM4.Enabled)
    {
        pDENMMsg4 = asn1_mallocz(sizeof(ITSDecentralizedEnvironmentalNotificationMessage));
    }

    if (pITS->Tx.DENM5.Enabled)
    {
        pDENMMsg5 = asn1_mallocz(sizeof(ITSDecentralizedEnvironmentalNotificationMessage));
    }
    ...
    while ((pITS->Tx.ThreadState & ITS_THREAD_STATE_STOP) == 0)
    {
        ...
        switch (RecvMsg.Id)
        {
        case QSMSG_ITS_DENM_EVENT_TX_REQ:
            DENMTx_SendDENMTrigger_Events(pITS);
            break;

        case QSMSG_ITS_DENM_EVENT_TERMINATE_TX_REQ:
            DENMTx_SendDENMTerminate_Events(pITS);
            break;
    ...
    if (pDENMMsg1 != NULL)
    {
        asn1_free_value(asn1_type_ITSDecentralizedEnvironmentalNotificationMessage, pDENMMsg1);
    }
    if (pDENMMsg2 != NULL)
    {
        asn1_free_value(asn1_type_ITSDecentralizedEnvironmentalNotificationMessage, pDENMMsg2);
    }
    if (pDENMMsg3 != NULL)
    {
        asn1_free_value(asn1_type_ITSDecentralizedEnvironmentalNotificationMessage, pDENMMsg3);
    }
    if (pDENMMsg4 != NULL)
    {
        asn1_free_value(asn1_type_ITSDecentralizedEnvironmentalNotificationMessage, pDENMMsg4);
    }
    if (pSpatData != NULL)
    {
 ...
 *
 */
static int DENMTx_FillDENMEvent(tITSTxDENMParams *pParams,
                                tDENMMgmt *pMgmt,
                                ...
                                const struct LPHData *pLPHData)
{
    ...
    // Check if we should use the GNSS or configured (hard coded) position for the event location
    if (pParams->EventPositionSource == ITS_CONFIG_VALUE_EVENTPOSITION_GNSS)
    {
    ...
    if (pParams->NumEventPoints > 0)
    {
        ...
        {
            d_printf(D_ERR, NULL, "Error allocating memory for eventHistory defining DENM segment\n");
            goto End;
        }
    ...
    if (pDENM->location_option)
    {
        ...
        if (pDENM->location.traces.tab == NULL)
        {
            d_printf(D_ERR, NULL, "Error allocating memory for Path History Trace to DENM\n");
        ...
        if (pPathHistory->tab == NULL)
        {
            d_error(D_ERR, NULL, "Error allocating memory for Path History to DENM\n");
            goto End;
        }
...
static int DENMTx_SendDENMTrigger_Events(struct ITSCtrl *pITS)
{
    ...
    d_printf(D_INFO, NULL, "Send DENM Event trigger\n");
    ...
    if (rc != 0)
    {
        d_error(D_ERR, NULL, "Could not access LPH GNSS information to populate DENM\n");
        goto End;
    }
    ...
    if (pITS->Tx.DENM1.Enabled)
    {
        ...
        DENMActionId1 = ETSIDENM_ACTIONID(pITS->Tx.DENM1.OrigStationId, pITS->Tx.DENM1.SeqNumber);

        Ret = DENMTx_FillDENMEvent(&pITS->Tx.DENM1, &DENMMgmt1, pDENMMsg1, pData);

        if (Ret)
        {
            d_printf(D_WARN, NULL, "Could not FillDENMEvent DENM1 (%d)\n", Ret);
            goto End;
        }
    }
    ...
    if (pITS->Tx.DENM2.Enabled)
    {
        ...
        DENMActionId2 = ETSIDENM_ACTIONID(pITS->Tx.DENM2.OrigStationId, pITS->Tx.DENM2.SeqNumber);

        Ret = DENMTx_FillDENMEvent(&pITS->Tx.DENM2, &DENMMgmt2, pDENMMsg2, pData);

        if (Ret)
        {
            d_printf(D_WARN, NULL, "Could not FillDENMEvent DENM2 (%d)\n", Ret);
            goto End;
        }
    }
    ...
    if (pITS->Tx.DENM3.Enabled)
    {
        ...
        DENMActionId3 = ETSIDENM_ACTIONID(pITS->Tx.DENM3.OrigStationId, pITS->Tx.DENM3.SeqNumber);

        Ret = DENMTx_FillDENMEvent(&pITS->Tx.DENM3, &DENMMgmt3, pDENMMsg3, pData);

        if (Ret)
        {
            d_printf(D_WARN, NULL, "Could not FillDENMEvent DENM3 (%d)\n", Ret);
            goto End;
        }
    }
    ...
    if (pITS->Tx.DENM4.Enabled)
    {
        ...
        DENMActionId4 = ETSIDENM_ACTIONID(pITS->Tx.DENM4.OrigStationId, pITS->Tx.DENM4.SeqNumber);

        Ret = DENMTx_FillDENMEvent(&pITS->Tx.DENM4, &DENMMgmt4, pDENMMsg4, pData);

        if (Ret)
        {
            d_printf(D_WARN, NULL, "Could not FillDENMEvent DENM4 (%d)\n", Ret);
            goto End;
        }
    }
    ...
    if (pITS->Tx.DENM5.Enabled)
    {
        ...
        DENMActionId5 = ETSIDENM_ACTIONID(pITS->Tx.DENM5.OrigStationId, pITS->Tx.DENM5.SeqNumber);

        Ret = DENMTx_FillDENMEvent(&pITS->Tx.DENM5, &DENMMgmt5, pDENMMsg5, pData);

        if (Ret)
        {
            d_printf(D_WARN, NULL, "Could not FillDENMEvent DENM5 (%d)\n", Ret);
            goto End;
        }
    }
    ...
    if (LastRoadWorkerTime == 0)
    {
        ...
        if ((pITS->Tx.UseRefDENMEnabled) && (NumDENM > 1))
        {
            if (pITS->Tx.DENM1.Enabled)
            {
                pDENMMsg1->alacarte.roadWorks.referenceDenms_option = true;
                pDENMMsg1->alacarte.roadWorks.referenceDenms.count = NumDENM - 1;
                pDENMMsg1->alacarte.roadWorks.referenceDenms.tab = (ITSActionID *)
                    asn1_mallocz(pDENMMsg1->alacarte.roadWorks.referenceDenms.count * sizeof(ITSActionID));

                int Count = 0;
                if (pITS->Tx.DENM2.Enabled)
                {
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId2);
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId2);
                }
                if (pITS->Tx.DENM3.Enabled)
                {
                    Count++;
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId3);
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId3);
                }
                if (pITS->Tx.DENM4.Enabled)
                {
                    Count++;
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId4);
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId4);
                }
                if (pITS->Tx.DENM5.Enabled)
                {
                    Count++;
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId5);
                    pDENMMsg1->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId5);
                }
            }

            if (pITS->Tx.DENM2.Enabled)
            {
                pDENMMsg2->alacarte.roadWorks.referenceDenms_option = true;
                pDENMMsg2->alacarte.roadWorks.referenceDenms.count = NumDENM - 1;
                pDENMMsg2->alacarte.roadWorks.referenceDenms.tab = (ITSActionID *)
                    asn1_mallocz(pDENMMsg2->alacarte.roadWorks.referenceDenms.count * sizeof(ITSActionID));

                int Count = 0;
                // The first RWW is not optional
                pDENMMsg2->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId1);
                pDENMMsg2->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId1);

                if (pITS->Tx.DENM3.Enabled)
                {
                    Count++;
                    pDENMMsg2->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId3);
                    pDENMMsg2->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId3);
                }
                if (pITS->Tx.DENM4.Enabled)
                {
                    Count++;
                    pDENMMsg2->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId4);
                    pDENMMsg2->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId4);
                }
                if (pITS->Tx.DENM5.Enabled)
                {
                    Count++;
            ...
            if (pITS->Tx.DENM3.Enabled)
            {
                pDENMMsg3->alacarte.roadWorks.referenceDenms_option = true;
                pDENMMsg3->alacarte.roadWorks.referenceDenms.count = NumDENM - 1;
                pDENMMsg3->alacarte.roadWorks.referenceDenms.tab = (ITSActionID *)
                    asn1_mallocz(pDENMMsg3->alacarte.roadWorks.referenceDenms.count * sizeof(ITSActionID));

                int Count = 0;
                // The first RWW is not optional
                pDENMMsg3->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId1);
                pDENMMsg3->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId1);

                if (pITS->Tx.DENM2.Enabled)
                {
                    Count++;
                    pDENMMsg3->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId2);
                    pDENMMsg3->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId2);
                }
                if (pITS->Tx.DENM4.Enabled)
                {
                    Count++;
                    pDENMMsg3->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId4);
                    pDENMMsg3->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId4);
                }
                if (pITS->Tx.DENM5.Enabled)
                {
                    Count++;
            ...
            if (pITS->Tx.DENM4.Enabled)
            {
                pDENMMsg4->alacarte.roadWorks.referenceDenms_option = true;
                pDENMMsg4->alacarte.roadWorks.referenceDenms.count = NumDENM - 1;
                pDENMMsg4->alacarte.roadWorks.referenceDenms.tab = (ITSActionID *)
                    asn1_mallocz(pDENMMsg4->alacarte.roadWorks.referenceDenms.count * sizeof(ITSActionID));

                int Count = 0;
                // The first RWW is not optional
                pDENMMsg4->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId1);
                pDENMMsg4->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId1);

                if (pITS->Tx.DENM2.Enabled)
                {
                    Count++;
                    pDENMMsg4->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId2);
                    pDENMMsg4->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId2);
                }
                if (pITS->Tx.DENM3.Enabled)
                {
                    Count++;
                    pDENMMsg4->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId3);
                    pDENMMsg4->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId3);
                }
                if (pITS->Tx.DENM5.Enabled)
                {
                    Count++;
            ...
            if (pITS->Tx.DENM5.Enabled)
            {
                pDENMMsg5->alacarte.roadWorks.referenceDenms_option = true;
                pDENMMsg5->alacarte.roadWorks.referenceDenms.count = NumDENM - 1;
                pDENMMsg5->alacarte.roadWorks.referenceDenms.tab = (ITSActionID *)
                    asn1_mallocz(pDENMMsg5->alacarte.roadWorks.referenceDenms.count * sizeof(ITSActionID));

                int Count = 0;
                // The first RWW is not optional
                pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId1);
                pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId1);

                if (pITS->Tx.DENM2.Enabled)
                {
                    Count++;
                    pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId2);
                    pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId2);
                }
                if (pITS->Tx.DENM3.Enabled)
                {
                    Count++;
                    pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId3);
                    pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId3);
                }
                if (pITS->Tx.DENM4.Enabled)
                {
                    Count++;
                    pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].originatingStationID = ETSIDENM_ACTIONID_STATION(DENMActionId4);
                    pDENMMsg5->alacarte.roadWorks.referenceDenms.tab[Count].sequenceNumber = ETSIDENM_ACTIONID_SEQNUM(DENMActionId4);
                }
            }
        }
        ...
        if (pITS->Tx.DENM1.Enabled)
        {
            status = DENM_SendDENMTriggerWithId(&DENMMgmt1, pDENMMsg1, DENMActionId1);
        }
        // Send optional second Denm trigger
        if (pITS->Tx.DENM2.Enabled)
        {
            status = DENM_SendDENMTriggerWithId(&DENMMgmt2, pDENMMsg2, DENMActionId2);
        }
        // Send optional third Denm trigger
        if (pITS->Tx.DENM3.Enabled)
        {
            status = DENM_SendDENMTriggerWithId(&DENMMgmt3, pDENMMsg3, DENMActionId3);
        }
        // Send optional fourth Denm trigger
        if (pITS->Tx.DENM4.Enabled)
        {
            status = DENM_SendDENMTriggerWithId(&DENMMgmt4, pDENMMsg4, DENMActionId4);
        }
        // Send optional fifth Denm trigger
        if (pITS->Tx.DENM5.Enabled)
        {
            status = DENM_SendDENMTriggerWithId(&DENMMgmt5, pDENMMsg5, DENMActionId5);
        }

        if (status == DENM_SUCCESS)
        {
            // update last DENM time
            ...
            LastRoadWorkerPos.Lat_Degs = (float)pDENMMsg1->management.eventPosition.latitude / LATLON_UNITS_PER_DEGREE;
            LastRoadWorkerPos.Lon_Degs = (float)pDENMMsg1->management.eventPosition.longitude / LATLON_UNITS_PER_DEGREE;
            LastTraceRoadWorkerPos.Lat_Degs = (float)pDENMMsg1->management.eventPosition.latitude / LATLON_UNITS_PER_DEGREE;
            LastTraceRoadWorkerPos.Lon_Degs = (float)pDENMMsg1->management.eventPosition.longitude / LATLON_UNITS_PER_DEGREE;
        ...
        {
            d_printf(D_WARN, NULL, "Triggering DENM failed: %d\n", status);
            Ret = -EAGAIN;
        }
    }
    else
    {
        ...
        CurrentPos.Lat_Degs = (float)pDENMMsg1->management.eventPosition.latitude / LATLON_UNITS_PER_DEGREE;
        CurrentPos.Lon_Degs = (float)pDENMMsg1->management.eventPosition.longitude / LATLON_UNITS_PER_DEGREE;
        d_printf(D_DEBUG, NULL, "CurPos.Lat = %f\n", CurrentPos.Lat_Degs);
        d_printf(D_DEBUG, NULL, "CurPos.Lon = %f\n", CurrentPos.Lon_Degs);
        ...
        d_printf(D_DEBUG, NULL,
                 ...
                 pITS->Tx.DENM1.Duration);

        if (((uint64_t)(Now - LastRoadWorkerTime) >=
             pITS->Tx.DENM1.UpdatePeriod) ||
            (DistanceFromLastUpdate >= 4))
        {
            // Send Denm update (with current ActionId)
            if (pITS->Tx.DENM1.Enabled)
            {
                status = DENM_SendDENMUpdate(&DENMMgmt1, pDENMMsg1, DENMActionId1);
            }
            // Send Optional second Denm update
            if (pITS->Tx.DENM2.Enabled)
            {
                status = DENM_SendDENMUpdate(&DENMMgmt2, pDENMMsg2, DENMActionId2);
            }
            // Send Optional third Denm trigger
            if (pITS->Tx.DENM3.Enabled)
            {
                status = DENM_SendDENMUpdate(&DENMMgmt3, pDENMMsg3, DENMActionId3);
            }
            // Send Optional fourth Denm trigger
            if (pITS->Tx.DENM4.Enabled)
            {
                status = DENM_SendDENMUpdate(&DENMMgmt4, pDENMMsg4, DENMActionId4);
            }
            // Send Optional fifth Denm trigger
            if (pITS->Tx.DENM5.Enabled)
            {
                status = DENM_SendDENMUpdate(&DENMMgmt5, pDENMMsg5, DENMActionId5);
            }
            ...
            {
                d_printf(D_WARN, NULL, "Updating DENM Failed %d\n", status);
            }
        }
        else
        {
            d_printf(D_INFO, NULL, "Not updating DENM\n");
            Ret = 0;
            goto End;
        }
    }
 ...
 * @return 0 for success
 */
void DENMTx_FillDENMTerminateEvent(tITSTxDENMParams *pParams,
                                   tDENMMgmt *pMgmt,
                                   ITSDecentralizedEnvironmentalNotificationMessage *pDenm)
{
 ...
 *
 */
int DENMTx_SendDENMTerminate_Events(struct ITSCtrl *pITS)
{
    ...
    if (pITS->Tx.DENM1.Enabled)
    {
        DENMTx_FillDENMTerminateEvent(&(pITS->Tx.DENM1), &DENMMgmt1, pDENMMsg1);

        // Send Denm
        tDENMStatusCode status = DENM_SendDENMTermination(&DENMMgmt1, pDENMMsg1, DENMActionId1);
    ...
    if (pITS->Tx.DENM2.Enabled)
    {
        DENMTx_FillDENMTerminateEvent(&(pITS->Tx.DENM2), &DENMMgmt2, pDENMMsg2);

        // Send Denm
        tDENMStatusCode status = DENM_SendDENMTermination(&DENMMgmt2, pDENMMsg2, DENMActionId2);
    ...
    if (pITS->Tx.DENM3.Enabled)
    {
        DENMTx_FillDENMTerminateEvent(&(pITS->Tx.DENM3), &DENMMgmt3, pDENMMsg3);

        // Send Denm
        tDENMStatusCode status = DENM_SendDENMTermination(&DENMMgmt3, pDENMMsg3, DENMActionId3);
    ...
    if (pITS->Tx.DENM4.Enabled)
    {
        DENMTx_FillDENMTerminateEvent(&(pITS->Tx.DENM4), &DENMMgmt4, pDENMMsg4);

        // Send Denm
        tDENMStatusCode status = DENM_SendDENMTermination(&DENMMgmt4, pDENMMsg4, DENMActionId4);
    ...
    if (pITS->Tx.DENM5.Enabled)
    {
        DENMTx_FillDENMTerminateEvent(&(pITS->Tx.DENM5), &DENMMgmt5, pDENMMsg5);

        // Send Denm
        tDENMStatusCode status = DENM_SendDENMTermination(&DENMMgmt5, pDENMMsg5, DENMActionId5);
