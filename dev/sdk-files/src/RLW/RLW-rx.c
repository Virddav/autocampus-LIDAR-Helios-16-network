                     ...
                     void *pPriv)
{
  ...
  bool CurrentAlertIsRLW;
  if (RLW.NA) 
  {
    CurrentAlertIsRLW = ((pCurrentAlert != NULL) &&
                         (pCurrentAlert->Type == TA_THREATTYPE_SVW));
  }
  else 
  {
    CurrentAlertIsRLW = ((pCurrentAlert != NULL) &&
...
static void RLW_PrepareAlertInfo(const tSPATMAPIntersection *pSpatMapIn)
{
  ...
  if (pSpatMapIn == NULL)
  {
    d_printf(D_NOTICE, NULL, "No valid data.\n");
  ...
  else
  {
    ...
    if (RLW.NA)
    {
       pTAAlert->Type              = TA_THREATTYPE_SVW;
    }  
    else
    {
