...
#define SET_BOOL(x)                                                           \
    do                                                                        \
    {                                                                         \
        if (config_lookup_bool(pConfig, "Example." #x ".Enable", &ConfigVal)) \
        {                                                                     \
                       ...
                       void *pParam)
{
    ...
    case TA_THREATTYPE_ICW:
    {
        ...
        else if (ThreatBearing > SafetyZone.NorthEast)
            // Right
            Direction = HMI_ALERT_DIRECTION_RIGHT;
        else if (ThreatBearing > SafetyZone.NorthWest)
            // Front (should not get here)
            Direction = HMI_ALERT_DIRECTION_UNKNOWN;
        else if (ThreatBearing > SafetyZone.SouthWest)
            // Left
            ...
            if (pConfig)
            {
                ...
                if (NumPC == 1 && strstr(pConfig->pText, "%s") != NULL)
                {
                    snprintf(Warning, sizeof(Warning), pConfig->pText,
                             (Direction == HMI_ALERT_DIRECTION_LEFT ? "Left" : (Direction == HMI_ALERT_DIRECTION_RIGHT ? "Right" : "")));
    ...
    case TA_THREATTYPE_RWW:
    {
        ...
        if (pAlert->RWW.RWWDirection == 1 /* RWW_BEHIND */)
        {
            ...
            {
                Len += snprintf(AlertStr + Len, sizeof(AlertStr) - Len,
                                "|%s|", (pAlert->RWW.InnerHardShoulderStatus == 2 ? "↑" : (pAlert->RWW.InnerHardShoulderStatus == 1 ? "T" : "_")));
            }
            int i;
            ...
            {
                Len += snprintf(AlertStr + Len, sizeof(AlertStr) - Len,
                                "|%s|", (pAlert->RWW.OuterHardShoulderStatus == 2 ? "↑" : (pAlert->RWW.OuterHardShoulderStatus == 1 ? "T" : "_")));
            }
