...
static int ITSCtrl_LoadDENMConfig(tITSTxDENMParams *pDENM,
                              ...
                              char *pConfigFileName)
{
    ...
    // Set defaults
    pITS->Tx.DENM1.Enabled = false;
    pITS->Tx.DENM2.Enabled = false;
    pITS->Tx.DENM3.Enabled = false;
    pITS->Tx.DENM4.Enabled = false;
    pITS->Tx.DENM5.Enabled = false;
    ...
    else
    {
                                       ...
                                       &ConfigVal))
        {
            ...
            if (DENMTxEnabled)
            {
                ...
                {
                    // Try (legacy) RWW
                    pEventConfig = config_setting_get_member(pDENM, ITS_CONFIG_VALUE_NAME_DENMRWW);
                }

                if (pEventConfig != NULL)
                {
                    Res = ITSCtrl_LoadDENMConfig(&pITS->Tx.DENM1, pEventConfig);
                    if (Res)
                    {
                        d_fnend(D_ERR, NULL, "ITSCtrl_LoadDENMConfig(): %d\n", Res);
                        goto Error;
                    }
                }
                else
                ...
                {
                    // Try (legacy) RWW2
                    pEventConfig = config_setting_get_member(pDENM, ITS_CONFIG_VALUE_NAME_DENMRWW2);
                }

                if (pEventConfig != NULL)
                {
                    Res = ITSCtrl_LoadDENMConfig(&pITS->Tx.DENM2, pEventConfig);
                    if (Res)
                    {
                        d_fnend(D_ERR, NULL, "ITSCtrl_LoadDENMConfig(): %d\n", Res);
                        goto Error;
                ...
                {
                    // Try (legacy) RWW3
                    pEventConfig = config_setting_get_member(pDENM, ITS_CONFIG_VALUE_NAME_DENMRWW3);
                }

                if (pEventConfig != NULL)
                {
                    Res = ITSCtrl_LoadDENMConfig(&pITS->Tx.DENM3, pEventConfig);
                    if (Res)
                    {
                        d_fnend(D_ERR, NULL, "ITSCtrl_LoadDENMConfig(): %d\n", Res);
                        goto Error;
                ...
                {
                    // Try (legacy) RWW4
                    pEventConfig = config_setting_get_member(pDENM, ITS_CONFIG_VALUE_NAME_DENMRWW4);
                }

                if (pEventConfig != NULL)
                {
                    Res = ITSCtrl_LoadDENMConfig(&pITS->Tx.DENM4, pEventConfig);
                    if (Res)
                    {
                        d_fnend(D_ERR, NULL, "ITSCtrl_LoadDENMConfig(): %d\n", Res);
                        goto Error;
                ...
                // Event5
                pEventConfig = config_setting_get_member(
                    pDENM, ITS_CONFIG_VALUE_NAME_DENMEVENT5);

                if (pEventConfig == NULL)
                {
                    // Try (legacy) RWW5
                    pEventConfig = config_setting_get_member(pDENM, ITS_CONFIG_VALUE_NAME_DENMRWW5);
                }

                if (pEventConfig != NULL)
                {
                    Res = ITSCtrl_LoadDENMConfig(&pITS->Tx.DENM5, pEventConfig);
                    if (Res)
                    {
                        d_fnend(D_ERR, NULL, "ITSCtrl_LoadDENMConfig(): %d\n", Res);
                        goto Error;
                ...
                {
                    d_printf(D_NOTICE, NULL, "'%s' has no '%s' section\n",
                             pDENM->name, ITS_CONFIG_VALUE_NAME_DENMEVENT5);
                }
...
/**
 * @brief Load the DENM-specific sections of the config file
 *
                                  ...
                                  config_setting_t *pSetting)
{
    ...
    memset(pDENM, 0, sizeof(tITSTxDENMParams));

    // Ensure subsequent DENM structures have unique default SeqNum values
    static int SeqExtra = 0;

    pDENM->Enabled = false;
    pDENM->Duration = ITS_CONFIG_VALUE_DEFAULT_DURATION;
    pDENM->OrigStationId = ITS_CONFIG_VALUE_DEFAULT_ORIGSTATIONID;
    pDENM->SeqNumber = ITS_CONFIG_VALUE_DEFAULT_SEQNUMBER + SeqExtra;
    pDENM->UpdatePeriod = ITS_CONFIG_VALUE_DEFAULT_UPDATEPERIOD;
    pDENM->RepInterval = ITS_CONFIG_VALUE_DEFAULT_REPINTERVAL;
    pDENM->EventOffsetAbsolute = ITS_CONFIG_VALUE_DEFAULT_EVENTOFFSETABSOLUTE;
    pDENM->RelativeOffsetCumulative = ITS_CONFIG_VALUE_DEFAULT_RELATIVEOFFSETCUMULATIVE;
    pDENM->EventPositionSource = ITS_CONFIG_VALUE_DEFAULT_EVENTPOSITIONSOURCE;
    pDENM->EventLatitude = ITS_CONFIG_VALUE_DEFAULT_EVENTLAT;
    pDENM->EventLongitude = ITS_CONFIG_VALUE_DEFAULT_EVENTLONG;
    pDENM->EventAltitude = ITS_CONFIG_VALUE_DEFAULT_EVENTALT;
    pDENM->TrafficClass = ITS_CONFIG_VALUE_DEFAULT_TRAFFICCLASS;
    pDENM->InformationQuality = ITS_CONFIG_VALUE_DEFAULT_INFORMATIONQUALITY;
    pDENM->CauseCode = ITS_CONFIG_VALUE_DEFAULT_CAUSECODE;
    pDENM->SubCauseCode = ITS_CONFIG_VALUE_DEFAULT_SUBCAUSECODE;
    pDENM->RelevanceTrafficDirection = ITS_CONFIG_VALUE_DEFAULT_TRAFFICDIRECTION;
    pDENM->RelevanceDistance = ITS_CONFIG_VALUE_DEFAULT_RELEVANCEDIST;
    pDENM->EventHeading = ITS_CONFIG_VALUE_DEFAULT_EVENTHEADING;

    // optional items not present by default
    pDENM->ClosedLanesOption = false;
    pDENM->SpeedLimitOption = false;
    pDENM->StartingPointSpeedLimitOption = false;
    pDENM->LanePositionOption = false;
    pDENM->TrafficFlowRuleOption = false;
    pDENM->EventHeadingOption = 0;

    // traces
    pDENM->TraceUpdateDistance_m = ITS_CONFIG_VALUE_DEFAULT_TRACEUPDATEDISTANCE_M;

    // These are optional, use illegal values
    pDENM->OuterHardShoulderStatus = ITSHardShoulderStatus_Limit_Max + 1;
    pDENM->InnerHardShoulderStatus = ITSHardShoulderStatus_Limit_Max + 1;

    pDENM->NumClosedLanes = 0;

    int ConfigVal = 0;
    // DENM Enabled
    if (config_setting_lookup_bool(pSetting,
                                   ITS_CONFIG_VALUE_NAME_DENMTXENABLED,
                                   &ConfigVal))
        pDENM->Enabled = (bool)ConfigVal;

    // Duration
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DURATION,
                                  &ConfigVal))
        pDENM->Duration = (uint16_t)ConfigVal;

    // OrigStationId
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMORIGSTATIONID,
                                  &ConfigVal))
        pDENM->OrigStationId = (uint32_t)ConfigVal;

    // SeqNumber
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMSEQNUMBER,
                                  &ConfigVal))
        pDENM->SeqNumber = (uint16_t)ConfigVal;

    // UpdatePeriod
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_UPDATEPERIOD,
                                  &ConfigVal))
        pDENM->UpdatePeriod = (uint16_t)ConfigVal;

    // RepInterval
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_REPINTERVAL,
                                  &ConfigVal))
        pDENM->RepInterval = (uint16_t)ConfigVal;
    // RelativeOffsetCumulative
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_RELATIVEOFFSETCUMULATIVE,
                                  &ConfigVal))
        pDENM->RelativeOffsetCumulative = (uint8_t)ConfigVal;
    // EventOffsetAbsolute
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_EVENTOFFSETABSOLUTE,
                                  &ConfigVal))
        pDENM->EventOffsetAbsolute = (uint8_t)ConfigVal;
    // EventPositionSource
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_EVENTPOSSOURCE,
                                  &ConfigVal))
        pDENM->EventPositionSource = (uint16_t)ConfigVal;

    // read position from the configuration file only if config source specified
    if (pDENM->EventPositionSource == ITS_CONFIG_VALUE_EVENTPOSITION_CONFIG)
    {
        ITSCtrl_ReadLatLong(&pDENM->EventLatitude,
                            &pDENM->EventLongitude,
                            &pDENM->EventAltitude,
                            pSetting,
    ...
    // else EventPositionSource is from GNSS and will be updated in FAC layer

    // TrafficClass
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_TRAFFICCLASS,
                                  &ConfigVal))
        pDENM->TrafficClass = (uint8_t)ConfigVal;

    // InformationQuality
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMINFORMATIONQUALITY,
                                  &ConfigVal))
        pDENM->InformationQuality = (uint8_t)ConfigVal;

    // CauseCode
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMCAUSECODE,
                                  &ConfigVal))
        pDENM->CauseCode = (uint8_t)ConfigVal;

    // SubCauseCode
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMSUBCAUSECODE,
                                  &ConfigVal))
        pDENM->SubCauseCode = (uint8_t)ConfigVal;

    // RelevanceTrafficDirection
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMRELEVANCETRAFFICDIRECTION,
                                  &ConfigVal))
        pDENM->RelevanceTrafficDirection = (uint8_t)ConfigVal;

    // RelevanceDistance
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMRELEVANCEDIST,
                                  &ConfigVal))
        pDENM->RelevanceDistance = (uint8_t)ConfigVal;

    // EventHeadingOption
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMEVENTHEADINGOPTION,
                                  &ConfigVal))
        pDENM->EventHeadingOption = (uint8_t)ConfigVal;

    // EventHeading
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMEVENTHEADING,
                                  &ConfigVal))
        pDENM->EventHeading = (uint16_t)ConfigVal;

    // ClosedLanesOption
    if (config_setting_lookup_bool(pSetting,
                                   ITS_CONFIG_VALUE_NAME_DENMCLOSEDLANESOPTION,
                                   &ConfigVal))
        pDENM->ClosedLanesOption = (bool)ConfigVal;

    // ClosedLanes
    config_setting_t *pClosedLanes = config_setting_get_member(pSetting, ITS_CONFIG_VALUE_NAME_DENMCLOSEDLANES);
    if (pClosedLanes)
    {
        int NumberOfLanes = config_setting_length(pClosedLanes);
        if (NumberOfLanes > 0)
        {
            pDENM->NumClosedLanes = NumberOfLanes;
            pDENM->ClosedLanes = (int *)calloc(sizeof(int), NumberOfLanes);
            if (!pDENM->ClosedLanes)
            {
                d_printf(D_ERR, NULL,
    ...
    // SpeedLimitOption
    if (config_setting_lookup_bool(pSetting,
                                   ITS_CONFIG_VALUE_NAME_DENMSPEEDLIMITOPTION,
                                   &ConfigVal))
        pDENM->SpeedLimitOption = (bool)ConfigVal;
    // SpeedLimit
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMSPEEDLIMIT,
                                  &ConfigVal))
        pDENM->SpeedLimit = (uint8_t)ConfigVal;

    // StartingPointSpeedLimitOption
    if (config_setting_lookup_bool(pSetting,
                                   ITS_CONFIG_VALUE_NAME_DENMSTARTINGPOINTSPEEDLIMITOPTION,
                                   &ConfigVal))
        pDENM->StartingPointSpeedLimitOption = (bool)ConfigVal;

    // StartingPointSpeedLimit
    {
        config_setting_t *pStartingPointSpeedLimitPoints =
            config_setting_get_member(
                pSetting, ITS_CONFIG_VALUE_NAME_DENMSTARTINGPOINTSPEEDLIMIT);
        if (pStartingPointSpeedLimitPoints != NULL)
        {
            ...
            {
                ITSCtrl_ReadPos(&pDENM->StartingPointSpeedLimit,
                                pStartingPointSpeedLimit,
                                ITS_CONFIG_VALUE_NAME_DENMLATITUDE,
                                ITS_CONFIG_VALUE_NAME_DENMLONGITUDE,
                                ITS_CONFIG_VALUE_NAME_DENMALTITUDE,
                                NULL);
            }
        }
    }
    ...
    if (config_setting_lookup_bool(pSetting,
                                   ITS_CONFIG_VALUE_NAME_DENMTRAFFICFLOWRULEOPTION,
                                   &ConfigVal))
        pDENM->TrafficFlowRuleOption = (bool)ConfigVal;

    // TrafficFlowRule
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMTRAFFICFLOWRULE,
                                  &ConfigVal))
        pDENM->TrafficFlowRule = (uint8_t)ConfigVal;

    // LanePositionOption
    if (config_setting_lookup_bool(pSetting,
                                   ITS_CONFIG_VALUE_NAME_LANEPOSITIONOPTION,
                                   &ConfigVal))
        pDENM->LanePositionOption = (bool)ConfigVal;

    // LanePosition
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_LANEPOSITION,
                                  &ConfigVal))
        pDENM->LanePosition = (uint8_t)ConfigVal;

    // OuterHardShoulderStatus
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMOUTERHARDSHOULDERSTATUS,
                                  &ConfigVal))
        pDENM->OuterHardShoulderStatus = (uint8_t)ConfigVal;

    // InnerHardShoulderStatus
    if (config_setting_lookup_int(pSetting,
                                  ITS_CONFIG_VALUE_NAME_DENMINNERHARDSHOULDERSTATUS,
                                  &ConfigVal))
        pDENM->InnerHardShoulderStatus = (uint8_t)ConfigVal;

    // Event History - Defines the DENM segments
    {
        config_setting_t *pEventPoints = config_setting_get_member(
            pSetting, ITS_CONFIG_VALUE_NAME_DENMEVENTHISTORY);
        if (pEventPoints != NULL)
        {
            int NumberOfEventPoints = config_setting_length(pEventPoints);
            pDENM->NumEventPoints = NumberOfEventPoints;
            if (NumberOfEventPoints > 0)
            {
                pDENM->pEventPoints = (tDENMReferencePoint *)calloc(sizeof(tDENMReferencePoint),
                                                                    NumberOfEventPoints);
                if (!pDENM->pEventPoints)
                {
                    d_printf(D_ERR, NULL,
                ...
                for (i = 0; i < NumberOfEventPoints; i++)
                {
                    ...
                    if (pEventPoint != NULL)
                    {
                        ITSCtrl_ReadPos(&pDENM->pEventPoints[i],
                                        pEventPoint,
                                        ITS_CONFIG_VALUE_NAME_DENMLATITUDE,
                                        ITS_CONFIG_VALUE_NAME_DENMLONGITUDE,
                                        ITS_CONFIG_VALUE_NAME_DENMALTITUDE,
    ...
    // Traces
    {
        ...
        if (config_setting_lookup_int(pSetting,
                                      ITS_CONFIG_VALUE_NAME_DENMTRACEUPDATEDISTANCE,
                                      &ConfigVal))
            pDENM->TraceUpdateDistance_m = (int16_t)ConfigVal;

        config_setting_t *pTraces = config_setting_get_member(
            pSetting, ITS_CONFIG_VALUE_NAME_DENMTRACES);
        if (pTraces != NULL)
        {
            int NumberOfTraces = config_setting_length(pTraces);
            if (NumberOfTraces > 0)
            {
                pDENM->NumTraces = NumberOfTraces;
                pDENM->pTraces = (tDENMReferencePoint *)calloc(sizeof(tDENMReferencePoint),
                                                               NumberOfTraces);
                if (!pDENM->pTraces)
                {
                    d_printf(D_ERR, NULL,
                ...
                for (i = 0; i < NumberOfTraces; i++)
                {
                    ...
                    {
                        // Latitude & Longitude
                        ITSCtrl_ReadPos(&pDENM->pTraces[i],
                                        pTrace,
                                        ITS_CONFIG_VALUE_NAME_DENMLATITUDE,
                                        ITS_CONFIG_VALUE_NAME_DENMLONGITUDE,
                                        ITS_CONFIG_VALUE_NAME_DENMALTITUDE,
                                 ...
                                 config_setting_t *pSetting)
{
    ...
    // Data
    config_setting_t *pData = config_setting_get_member(pSA, ITS_CONFIG_VALUE_NAME_SADATA);
    if (pData)
    {
                                   ...
                                   config_setting_t *pSetting)
{
    ...
    // Data
    config_setting_t *pData = config_setting_get_member(pRTCM, ITS_CONFIG_VALUE_NAME_RTCMDATA);
    if (pData)
    {
        int DataLen = config_setting_length(pData);
        if (DataLen > 0)
...
static int ITSCtrl_UnloadConfig(struct ITSCtrl *pITS)
{
    ...
    if (pITS)
    {
        ...
        pITS->Tx.DENM1.Enabled = false;
        free(pITS->Tx.DENM1.pTraces);
        free(pITS->Tx.DENM1.ClosedLanes);
        free(pITS->Tx.DENM1.pEventPoints);

        pITS->Tx.DENM2.Enabled = false;
        free(pITS->Tx.DENM2.pTraces);
        free(pITS->Tx.DENM2.ClosedLanes);
        free(pITS->Tx.DENM2.pEventPoints);

        pITS->Tx.DENM3.Enabled = false;
        free(pITS->Tx.DENM3.pTraces);
        free(pITS->Tx.DENM3.ClosedLanes);
        free(pITS->Tx.DENM3.pEventPoints);

        pITS->Tx.DENM4.Enabled = false;
        free(pITS->Tx.DENM4.pTraces);
        free(pITS->Tx.DENM4.ClosedLanes);
        free(pITS->Tx.DENM4.pEventPoints);

        pITS->Tx.DENM5.Enabled = false;
        free(pITS->Tx.DENM5.pTraces);
        free(pITS->Tx.DENM5.ClosedLanes);
        free(pITS->Tx.DENM5.pEventPoints);
...
static void ITSCtrl_ThreadProc(void *pArg)
{
    ...
    pITS->Tx.DENM1.NextTxTimeMS = TimeMSec;
    ...
    // Auto DENM station ID
    if ((pITS->Tx.DENM1.OrigStationId == 0) ||
        (pITS->Tx.DENM2.OrigStationId == 0) ||
        (pITS->Tx.DENM3.OrigStationId == 0) ||
        (pITS->Tx.DENM4.OrigStationId == 0) ||
        (pITS->Tx.DENM5.OrigStationId == 0))
    {
        // Zeroed stationID config replaced with 'current' station ID
        uint32_t LocalStationId = CAM_GetCurrentInfo().stationID;

        if (pITS->Tx.DENM1.OrigStationId == 0)
        {
            pITS->Tx.DENM1.OrigStationId = LocalStationId;
        }
        if (pITS->Tx.DENM2.OrigStationId == 0)
        {
            pITS->Tx.DENM2.OrigStationId = LocalStationId;
        }
        if (pITS->Tx.DENM3.OrigStationId == 0)
        {
            pITS->Tx.DENM3.OrigStationId = LocalStationId;
        }
        if (pITS->Tx.DENM4.OrigStationId == 0)
        {
            pITS->Tx.DENM4.OrigStationId = LocalStationId;
        }
        if (pITS->Tx.DENM5.OrigStationId == 0)
        {
            pITS->Tx.DENM5.OrigStationId = LocalStationId;
        }
    }
    ...
    while ((pITS->Ctrl.ThreadState & ITS_THREAD_STATE_STOP) == 0)
    {
        ...
        if (((pITS->Tx.DENM1.Enabled) || (pITS->Tx.DENM2.Enabled) || (pITS->Tx.DENM3.Enabled) || (pITS->Tx.DENM4.Enabled) || (pITS->Tx.DENM5.Enabled)) &&
            (NowMSec >= pITS->Tx.DENM1.NextTxTimeMS))
        {
            d_printf(D_INFO, 0, "Sending DENM %x\n", QSMSG_ITS_DENM_EVENT_TX_REQ);
            (void)Msg_SendId(QSMSG_ITS_DENM_EVENT_TX_REQ, QSQUEUE_ITSTX, QSPRIORITY_DEFAULT);
            pITS->Tx.DENM1.NextTxTimeMS += max(ITSCTRL_NAP_INTERVAL, pITS->Tx.DENM1.RepInterval);
        }
    ...
    if ((pITS->Tx.DENM1.Enabled) || (pITS->Tx.DENM2.Enabled) || (pITS->Tx.DENM3.Enabled) || (pITS->Tx.DENM4.Enabled) || (pITS->Tx.DENM5.Enabled))
    {
        (void)Msg_SendId(QSMSG_ITS_DENM_EVENT_TERMINATE_TX_REQ,
