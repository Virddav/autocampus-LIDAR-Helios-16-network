    ...
    with open(log_file_path, 'r') as file:
        for line in file:
            if "[AOC] Rx statistics" in line:
                ...
                # Assuming no other characters after "34:120"
                px_string = line[start_index:].strip().split('\t')[0]
...
def calcul_information(log_file_path, packets_size):
    ...
    last_message = max(
        values_lists, key=lambda x: x[2]) if values_lists else None
    first_message = min(
        values_lists, key=lambda x: x[2]) if values_lists else None
    ...
    debit = 8 * ((float(last_message[1]) *
                 packets_size)/total_time)/(1000 * 1024)
...
def extracts_all(path, intervale, packets_size):
    ...
    for i in range(len(intervale)):
        log_file_path = path + '/small_' + str(intervale[i])
...
def create_plot(intervale, packets_size, protocol, info, color):
    data_droid3 = [sublist[info] for sublist in extracts_all(
        '../small_logs/droid3_'+str(packets_size)+'_'+protocol, intervale, packets_size)]
    data_droid4 = [sublist[info] for sublist in extracts_all(
        '../small_logs/droid4_'+str(packets_size)+'_'+protocol, intervale, packets_size)]
    data = []

    for a, b in zip(data_droid3, data_droid4):
        data.append((a+b)/2)

    plt.plot(intervale, data, marker='o', linestyle='-', color=color,
             label=protocol.upper() + ": " + str(packets_size) + " bytes", linewidth=1, markersize=2)
...
def main():
    ...
    # plt.title(
    #    "Débit en fonction de l'intervale d'envoi des messages et suivant la taille des packets")
    plt.xlabel('Tx Interval (ms)')
    ...
    create_plot(intervale_1048_dsrc, 1048, 'dsrc', 1, 'black')
    ...
    plt.figure() # figsize=(20, 10)
    # plt.title(
    #     "Taux d'erreur en fonction de l'intervale d'envoi des messages et suivant la taille des packets")
    plt.xlabel('Tx Interval (ms)')
    plt.ylabel('PDR (%)')
    plt.tight_layout()
    ...
    create_plot(intervale_1048_dsrc, 1048, 'dsrc', 2, 'black')
